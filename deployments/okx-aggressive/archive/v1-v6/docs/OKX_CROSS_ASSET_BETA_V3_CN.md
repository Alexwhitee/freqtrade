...
> 归档说明（2026-07-31）：V3 已停止作为独立候选运行。本文中的路径与命令
...
仓位计算明确区分两种资金口径：0.75% 风险预算和 15 USDT 储备使用未经过
`tradable_balance_ratio` 折减的账户总权益；Freqtrade 传入的 `max_stake`
继续作为折减后的实际下单容量上限。这样 30 USDT 账户只预留一次 15 USDT，
不会因配置中的 0.50 可交易比例再次把仓位压成 0。
...
修复重复资金预留后，在正式 `tradable_balance_ratio=0.50` 配置下，真实
OKX 执行样本从 0 笔恢复为 13 笔。2026-04-22 至 2026-07-27 的可比结果为：

- 总收益 -0.497 USDT / -1.66%；
- PF 0.24，闭合交易 Sharpe -1.09；
- 最大回撤 1.66%；
- 8 胜 5 负，全部为多头；
- 4 次完整止损合计 -0.653 USDT。

这证明 0 笔问题已经修复，但不构成收益门槛通过。曾测试“Beta 状态恶化立即
退出”和“单品种止损后 48 小时隔离”，结果恶化为 16 笔、-2.15%，因此没有
保留，避免把短样本中的无效规则固化到正式策略。

## 发布门槛

当前没有通过 2026 收益/PF/Sharpe 门槛，也没有足够的多年真实研究数据、
成本压力结果，或
